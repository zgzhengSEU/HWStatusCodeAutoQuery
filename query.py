...
def queryStatus(hwuid, password, email, passwd, host_server, deviceFingerInfo):
    ...
    print('\n[', datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '] ', "执行一次查询")
...
if __name__ == "__main__":
    if "HWUID" in os.environ:
        ...
        if hwuid == '':
            print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 HWUID ！")
            sys.exit(1) 
    else:
        print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 HWUID ！")
        sys.exit(1)
    ...
    if "PASSWORD" in os.environ:
        ...
        if password == '':
            print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 PASSWORD ！")
            sys.exit(1) 
    else:
        print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 PASSWORD ！")
        sys.exit(1)
    ...
    if "EMAIL" in os.environ:
        ...
        if your_email == '':
            print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 EMAIL ！")
            sys.exit(1) 
    else:
        print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 EMAIL ！")
        sys.exit(1)
    ...
    if "EMAILCODE" in os.environ:
        ...
        if email_password == '':
            print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 EMAILCODE ！")
            sys.exit(1) 
    else:
        print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "未找到 EMAILCODE ！")
        sys.exit(1)
    ...
    if "NOTIFY" in os.environ:
        ...
        if notify.lower() == "false":
            notify = False
            print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "关闭功能：每日定时启动脚本时，发送提醒脚本运行成功邮件")
        else:
            notify = True
            print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "开启功能：每日定时启动脚本时，发送提醒脚本运行成功邮件")
    else:
        notify = True
        print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "开启功能：每日定时启动脚本时，发送提醒脚本运行成功邮件")
    ...
    print("[", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "] ", "当前job成功运行5小时，切换下一个job")
